...
def get_predictions(trainer, model, split='test', char_threshold=0.5, dx_threshold=0.5):
    """
    ...
    for preds in predictions:
        diagnosis_predictions, characteristics_predictions, attributions, y_dx, y_char, image_id, x = preds
        
        df_img = pd.DataFrame(image_id, columns=['image_id'])
        
        df_true = pd.DataFrame(y_char, columns=char_class_labels)
        df_score = pd.DataFrame(characteristics_predictions, columns=char_class_labels_score)

        y_pred = torch.where(characteristics_predictions >= char_threshold, 1, 0)
        df_pred = pd.DataFrame(y_pred, columns=char_class_labels_pred)
        
        df_dx_score = pd.DataFrame(diagnosis_predictions, columns=['dx_pred'])

        df = pd.concat([df_img, df_true, df_pred, df_score, df_dx_score], axis=1)
